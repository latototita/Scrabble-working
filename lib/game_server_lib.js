  ...
  // sets up a linked list to evaluate words; only updates tiles used this turn;
	...
	// returns true if all words are valid, OTHERWISE returns the invalid word
		...
		bestWord: function() {
	      ...
	      // TODO: Check if there is junk in first or second index
      		...
      		if (!this.getTile(targetId).character) {
	      		for (var k = 0; k < NUM_TRAY_TILES; k++) {
	      			if (this.getTile(currentTurnTiles[k]).character === bestWord[j]) {
	      				var trayTile = currentTurnTiles[k];
	      				if (trayTile.coord != null) { 
		      				continue; 
		      			}
	      				this.tileMoved(trayTile, targetId, true);
	      				currentPlayer.emit('bestWordSet', {
	      					targetId: targetId,
	      					sourceId: trayTile.replace(currentPlayer.id, "")
	      				});
	      				break;
	      			}
	      		}
