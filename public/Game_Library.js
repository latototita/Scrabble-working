...
socket.on('connect', function(){
	...
	socket.on('tile_drop_received', function(data) {
		BoardUtil.tileDropped(data.sourceId, data.targetId, false);
	});
...
var BoardUtil = (function() {
	...
	return {
		...
		setTile: function(_target, _tile) {
			...
			_tile.id = id;
			tileMap.set(id, [_tile, element.offset()]);
			if (char != null) element.html("<p>" + char + "</p>").show();
		},
		...
		setTileDroppable: function(_target) {
			...
			element.droppable({
				drop: function(event, ui) {
					BoardUtil.tileDropped(ui.draggable.attr('id'), $(this).attr('id'), true);
				}
			});
		},

		tileDropped: function(sourceId, targetId, isSourceClient) {	
			if (BoardUtil.hasTile(targetId)) { 
				return; // space is occupied -- do nothing.
			}

			$("#" + sourceId).offset(BoardUtil.getOriginalOffset(sourceId));
			...
			if (isSourceClient) {
				socket.emit('tile_drop_received', {
					targetId: targetId,
					sourceId: sourceId
				});
			}
