    ...
    BoardUtil: (function(io, room) {
      let dictionary = {},
          ...
          tileBase = {
            ...
            O: { count: 12,  score: 1,  character: 'O' },
      ...
      return {
        ...
        getCurrentBoardTiles: function() {
          let currentBoardTileList = [];
          for (let i = 0; i < NUM_TRAY_TILES; i++) {
            if (!this.getIdFromArg(currentTurnTiles[i]).includes(currentPlayer.id)) { // get current turn tiles that are not
                currentBoardTileList.push(currentTurnTiles[i]); // in the tray
            }
          }
        ...
        /*
         * Checks that the user has placed tiles correctly -- this means they are contiguous,
         * in the same column or row, and connected to previously placed tiles (or is on the center tile)
         */
        evaluateTilePlacementValidity: function() {
          ...
          let count = 0,
              isConnected = false,
              traverse = this.getTile(tilePos[0]);

          const checkValidity = traverse => {
        ...
        /*
         * Calculates the score by recusively traversing each perpendicular word created for each new tile
         * placed in the created word
         */
        ...
        calcScore: function(tileAlignment) {
          ...
          if (!traverse) {
            console.error(currentTurnTiles)
          }
        ...
        bestWord: function() {
          if (
            ...
            let horizontalWords = new Array(NUM_COLUMNS);
            for (let row = 0; row < NUM_ROWS; row++) {
              let verticalWord = '';
              for (let column = 0; column < NUM_COLUMNS; column++) {
                let char = this.getTile([row, column]).character;
                verticalWord += char ? char : ' ';
                horizontalWords[i] = !horizontalWords[i] ? char : horizontalWords[i] + char;
              }
              let strToPush = 'r' + row + (row < 10 ? '-' : '') + verticalWord; // For python parsing
            ...
            PythonShell.run('best_word.py', options, (err, results) => {
              ...
              for (let i = 0; i < results.length; i++) {
                let startPos = parseInt(results[i++]),
                    orientation = results[i++],
                    word = results[i],
                    score = false,
                    alignment = orientation[0] == 'r' ? 'row' : 'column',
                    sector = orientation[2] == '-' ? orientation[1] : orientation[1] + orientation[2];

                sector = parseInt(sector);

                for (let j = 0; j < word.length; j++) {
                  let row = alignment == 'row' ? sector : startPos + j,
                      column = alignment == 'column' ? sector : startPos + j,
                      targetId = 'tile' + row + '-' + column;
              ...
              for (let j = 0; j < bestWord.length; j++) {
                let row = bestAlignment == 'row' ? bestSector : bestStartPos + j,
                    column = bestAlignment == 'column' ? bestSector : bestStartPos + j,
                    targetId = 'tile' + row + '-' + column
