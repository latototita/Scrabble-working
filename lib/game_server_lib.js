  ...
  // sets up a linked list to evaluate words; only updates tiles used this turn;
			...
			if (isInRow) {
				...
				while(traverse && traverse.character) {
					if (this.isCurrentTurnTile(traverse.id)) {
						count++;
						if (traverse.coord[0] == 7 && traverse.coord[1] == 7
									|| traverse.up.character && !this.isCurrentTurnTile(traverse.up.id)
									|| traverse.left.character && !this.isCurrentTurnTile(traverse.left.id)
									|| traverse.down.character && !this.isCurrentTurnTile(traverse.down.id)
									|| traverse.right.character && !this.isCurrentTurnTile(traverse.right.id)) 
						{
							isConnected = true
						}
					} 
	...
	// returns true if all words are valid, OTHERWISE returns the invalid word
		...
		bestWord: function() {
	      ...
	      scriptPath: 'C:/Users/Ben/Desktop/scrabble/Scrabble/lib/',
	      args: pyArgs
	    }; 

			PythonShell.run('best_word.py', options, function(err, results){
				if (err) throw err;
	      // results is an array consisting of messages collected during execution
	      var maxScore = 0;
	      var startPos, orientation, word;
	      for (var i = 2; i < results.length; i += 3) {
	      	var score = BoardUtil.calcScore(results[i])
	      	if (score === true && score > maxScore) {
	      		maxScore = score;
	      		startPos = results[i-2];
	      		orientation = results[i-1];
	      		word = results[i];
	      	}
	      }
	      if (maxScore == 0) {
      	...
      	var alignment = orientation[0];
      	var sector = orientation[2] === '-' ? orientation[1] : orientation[1] + orientation[2];
      	sector = parseInt(sector);

      	for (var j = 0; j < word.length; j++) {
      		var targetId = 'tile'; 
      		var row 	 = alignment === 'r' ? sector : startPos + j;
      		var column = alignment === 'c' ? sector : startPos + j;

      		targetId += row + '-' + column;

      		if (!BoardUtil.getTile(targetId).character) {
	      		for (var k = 0; k < NUM_TRAY_TILES; k++) {
	      			if (BoardUtil.getTile(currentTurnTiles[k]).character === word[j]) {
	      				BoardUtil.tileMoved(currentTurnTiles[k], targetId);
	      			}
	      		}
	      	}
      	}
			...
			/*var BestWordScript = new PythonShell('best_word.py');

			//PythonShell.run('best_word.py', function (err, results))
			BestWordScript.run();
			BestWordScript.on('message', function(message) {
				console.log(message);
			});

			BestWordScript.end(function (err) {
			  if (err) throw err;
