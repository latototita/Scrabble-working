    ...
    init: function() {
      ...
      for (var i = 0; i < NUM_TRAY_TILES; i++) {
        ...
        tray.append(
          "<div id='tray-tile" +
            i +
            "' class='tile tray-tile current-turn-tiles'></div>"
        )
      }
...
var BoardUtil = (function() {
  ...
  return {
    ...
    tileDropped: function(sourceId, targetId) {
      ...
      console.log(sourceId + ' dropped. ' + targetId + ' now draggable')
    },
    ...
    getDomFromArg: function(callerParam) {
      // Allows methods to take in multiple types
    ...
    parseCoordFromId: function(str) {
      ...
      for (var i = 4; i < str.length; i++) {
        //indices specific to tile naming convention
    ...
    getPosFromArg: function(callerParam) {
      ...
      switch (type) { // Gets last two characters in ID; this will always be coordinates
        case 'string':
          return this.parseCoordFromId(callerParam)
        case 'array':
          return callerParam
        case 'object':
          return this.parseCoordFromId(callerParam.attr('id'))
      }
    },
