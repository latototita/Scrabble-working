...
BoardUtil : (function() {
  ...
  // sets up a linked list to evaluate words; only updates tiles used this turn;
		...
		evaluateTilePlacementValidity: function() {
			...
			var boardTiles = this.getCurrentBoardTiles();
			...
			for (var i = 0; i < tilePos.length; i++) {
				if (tilePos[i][0] != tilePos[0][0])
					isInRow = false;
				if (tilePos[i][1] != tilePos[0][1])
					isInColumn = false;
			}
			...
			if (!isConnected) {
				if (DEBUG) console.log("evaluateTilePlacementValidity(): Tiles placed not connected to existing tiles");
				return false;
			}
