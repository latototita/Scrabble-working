    ...
    BoardUtil: (function() {
      ...
      return {
        ...
        evaluateTilePlacementValidity: function() {
          ...
          var isConnectedTile = (tile) => {
            return tile
                   && tile.character
                   && !this.isCurrentTurnTile(tile.id);
          }
            ...
            while (traverse && traverse.character) {
              if (this.isCurrentTurnTile(traverse.id)) {
                ...
                if (
                  ...
                  || isConnectedTile(traverse.up)
                  || isConnectedTile(traverse.down)
                  || isConnectedTile(traverse.left)
                  || isConnectedTile(traverse.right)
                ) {
                  isConnected = true;
                }
              }
          ...
          if (count < numCurTilesOnBoard) {
            if (DEBUG)
              console.log(
                "evaluateTilePlacementValidity(): iterative count less than actual count"
              );
            return false;
          }
          ...
          if (!isConnected) {
            if (DEBUG)
              console.log(
                "evaluateTilePlacementValidity(): Tiles not connected"
              );
            return false;
          }
        ...
        calcScore: function(tileAlignment) {
          ...
          var result = this.calcScoreNittyGritty(
            traverse,
            tileAlignment,
            score,
            wordList
        ...
        bestWord: function() {
          ...
          var options = {
            mode: "text",
            pythonPath:
              "C:/Users/Ben/AppData/Local/Programs/Python/Python36/python.exe",
            pythonOptions: ["-u"],
